...
def main() -> int:
    ...
    question_path = eval_dir / "questions_direct10_lenient_20260615.json"
    questions = json.loads(question_path.read_text(encoding="utf-8"))
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    raw_path = eval_dir / f"three_agent_direct10_lenient_raw_answers_{timestamp}.jsonl"
    report_path = eval_dir / f"three_agent_direct10_lenient_scored_report_{timestamp}.md"
...
def score_record(record, criteria) -> None:
    ...
    for criterion in criteria:
        ...
        # Lenient rule: one clear alias hit is enough for the criterion.
